...
/**
 * Smarty modifier to convert multienum separated value to Array for the tags module
 *
 * Type:     function<br>
 * Name:     tag_multienum_to_array<br>
 * Purpose:  Utility to transform multienum String to Array format for tags
 */
function smarty_function_tag_multienum_to_array($params, &$smarty)
{
	$ret = "";
	if(empty($params['string'])) {
        ...
        {
            $tagNames = unencodeMultienum($params['string']);
            $module = $params['module'];
            $tagObj = BeanFactory::newBean('tag_Tags');
    	    $ret = $tagObj->getTagIdsFromNames($tagNames, $module, false);
        }
    }
